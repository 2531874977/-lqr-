...
# 车辆几何信息
WB = config.Vehicle_Length # rear to front wheel
W = config.Vehicle_Width   # width of car
LF = config.LF  # distance from rear to vehicle front end
LB = config.LB  # distance from rear to vehicle back end
MAX_STEER = config.MAX_STEER  # [rad] maximum steering angle
...
XY_GRID_RESOLUTION = config.XY_GRID_RESOLUTION  # [m]
YAW_GRID_RESOLUTION = config.YAW_GRID_RESOLUTION  # [rad]

N_STEER = config.N_STEER              # number of steer command
MOTION_RESOLUTION = config.MOTION_RESOLUTION  # [m] path interpolate resolution

SB_COST = config.SB_COST          # switch back penalty cost
BACK_COST = config.BACK_COST         # backward penalty cost
STEER_CHANGE_COST = config.STEER_CHANGE_COST  # steer angle change penalty cost
NON_STRAIGHT_COST = config.NON_STRAIGHT_COST  # steer angle not zero cost
H_COST = config.H_COST             # Heuristic cost
M_COST = config.M_COST            # Cost map coeffient

show_animation = config.show_animation

SB_COST = config.SB_COST          # switch back penalty cost
BACK_COST = config.SB_COST         # backward penalty cost
STEER_CHANGE_COST = config.STEER_CHANGE_COST  # steer angle change penalty cost
STEER_COST = config.STEER_COST       # steer angle not zero cost
H_COST = config.H_COST            # Heuristic cost
...
def move(x, y, yaw, distance, steer):
    x += distance * cos(yaw)
    y += distance * sin(yaw)
    yaw += pi_2_pi(distance * tan(steer) / WB)  # distance/2
...
# 由于车辆的运动轨迹只由质点表示，因此还需要对每一个路径点进行额外的碰撞检测。首先我们利用 KDtree
# 获取车辆外接圆范围内的所有障碍物，然后对障碍物检查是否在车辆的矩形内。
...
class Config:
    ...
    def __init__(self, ox, oy, xy_resolution, yaw_resolution):
        ...
        self.min_yaw = round(- pi / yaw_resolution) - 1
        self.max_yaw = round(pi / yaw_resolution)
...
def analytic_expansion(current, goal, ox, oy, kd_tree):
    ...
    max_curvature = tan(MAX_STEER) / WB / 1.5
...
# code test
def obstacleListInit():  # freespace 确定
    ox, oy = [], []
...
def main():
    ...
    ox, oy = obstacleListInit()

    # Set Initial parameters
    start = [10.0, 40.0, np.deg2rad(0)]
    goal = [53.0, 40.0, np.deg2rad(-90)]
    ...
    if show_animation:
        ...
        for i_x, i_y, i_yaw in zip(x, y, yaw):
            ...
            plt.grid(True)
            plt.axis("equal")
            car.plot_car(i_x, i_y, i_yaw)
            plt.pause(1e-6)
    else:
        plt.cla()
        plt.plot(ox, oy, "sk")
        car.plot_arrow(start[0], start[1], start[2], length=3)
        car.plot_arrow(goal[0], goal[1], goal[2], length=3)
        plt.plot(x, y, "-r", label="Hybrid A* path")
        plt.grid(True)
        plt.axis("equal")
        plt.show()
